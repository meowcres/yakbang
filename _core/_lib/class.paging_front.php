...
class paging_front{
	...
	function paging_front($url,$queryString,$offset,$page_block,$total_record,$page,$get_values){
		$this->url          = $url ;
    ...
    // 처음으로
		...
		if($this->first_block > $this->page_block){
			...
			echo "<a href='".$pre_url."' class='prevBtn'></a>" ;
		} else {
      echo "<a href='javascript:void(0);' class='prevBtn' onClick=\"alert('처음입니다')\"></a>" ;
    }
      ...
      echo "<a href='javascript:void(0);' class='nextBtn' onClick=\"alert('마지막 입니다')\"></a>" ;
    }
