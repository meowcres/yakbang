...
?>
    ...
    <div class="adm_table_style02">
				...
				<tr>
					...
					<td>
						<select id="keyfield" name="keyfield">
							<option value="t2.LICENSE_NUMBER" <? if ($search["keyfield"] == "t2.LICENSE_NUMBER") echo "selected"; ?>>약사번호</option>
							<option value="t1.USER_ID" <? if ($search["keyfield"] == "t1.USER_ID") echo "selected"; ?>>아이디</option>
							<option value="t1.USER_NAME" <? if ($search["keyfield"] == "t1.USER_NAME") echo "selected"; ?>>이름</option>
							<option value="t2.USER_PHONE" <? if ($search["keyfield"] == "t2.USER_PHONE") echo "selected"; ?>>연락처</option>
						</select>
						<input type="text" id="keyword" name="keyword" value="<?= $search["keyword"] ?>" class="w40p">
						<a onclick="sfrm.submit()" class="btn_s btn02">검색</a>
						<input type="button" value="초기화" class="btn_s btn02 btn_mgn" onClick="location.href='./popup_pharmacist_register.php?p_code=<?=$p_code?>'"> &nbsp;
					</td>
				</tr>
			</tbody>			
			</form>
        ...
        <table class="tbl_01">
            ...
            if ($totalnum > 0) {
                ...
                while ($row_002 = $db->sql_fetch_array($res_002)) {
                    ...
                    $user_sex = $row_002['USER_SEX'] == "M" ? "남성" : "여성" ;
                    ...
                    <tr>
                        ...
                        <td <?=$_class?>><?=$user_name?></td>
                        <td class="center" <?=$_class?>><?=$user_sex?></td>
                        <td class="center" <?=$_class?>>
                            <?php
                            if ($row_003[0] > 0) {
                                ?><a onclick="" class="btn_s btn14">불가</a><?
                            } else {
                                ?><a onclick="add_pp('<?=$user_id?>','<?=$p_code?>');" class="btn_s btn13">신청</a><?
                            }
                            ?>
                        </td>
                    </tr>
        ...
        <div class="b_page_no">
            <?php
            $paging = new paging("./popup_pharmacist_register.php","p_code=".$p_code,$offset,$page_block,$totalnum,$page,$url_opt);
            $paging->pagingArea("","") ;
            ?>
        </div>
    </div>
    ...
    <script>
        ...
        function add_pp(user_id, p_code){
            if(confirm("전문약사를 신청 하시겠습니까?")){
                var url = "../_action/pharmacist.do.php?Mode=add_pharmacist&user_id="+user_id+"&p_code="+p_code ;
