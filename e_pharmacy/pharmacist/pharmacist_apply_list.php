...
$qry_order = " ORDER BY t1.REG_DATE DESC" ;
...
<div id="content">
    <div class="sub_tit">약사관리 > 약사등록신청</div>
    <div id="cont">
        <div class="adm_cts">
                    ...
                    <table>
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                <select id="search_sex" name="search_sex" class="wid10" onChange="srFrm.submit();">
                                    ...
                                    <option value="M" <?php if ($search["sex"] == "M") {echo 'selected';}?>>남성</option>
                                    <option value="F" <?php if ($search["sex"] == "F") {echo 'selected';}?>>여성</option>
                                </select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                                <b>유형</b> &nbsp;:&nbsp;
                                <select id="search_region" name="search_region" class="wid10" onChange="srFrm.submit();">
                                    <option value="">전체</option>
                                    <option value="i" <?php if ($search["region"] == "i") {echo 'selected';}?>>메인약사</option>
                                    <option value="o" <?php if ($search["region"] == "o") {echo 'selected';}?>>협동약사</option>
                                </select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td colspan="3">
                                <select name="keyfield" class="wid15">
                                    <option value="t1.MM_NAME"  <?php if($search["keyfield"] == "t1.MM_NAME")  { echo 'selected'; }?>>이름</option>
                                    <option value="t1.MM_EMAIL" <?php if($search["keyfield"] == "t1.MM_EMAIL") { echo 'selected'; }?>>이메일</option>
                                    <option value="t2.MM_HP"    <?php if($search["keyfield"] == "t2.MM_HP")    { echo 'selected'; }?>>연락처</option>
                                    <option value="t2.MM_HP"    <?php if($search["keyfield"] == "t2.MM_HP")    { echo 'selected'; }?>>약사면허</option>
                                </select>
                        ...
                        <tr>
                            ...
                            <td colspan="3">
                                ...
                                <a class="btn btn01" onclick="location.href='./pharmacy.template.php?slot=<?=$_slot?>&type=<?=$_type?>'">초기화</a>&nbsp;&nbsp;
                                <a class="btn btn21" onclick="openWin('./pharmacist/popup_pharmacist_register.php?p_code=<?=$_pharmacy["code"]?>','pharmacist_register',800,700,'scrollbars=yes')">약사등록신청</a>
                            </td>
                        </tr>
                        </tbody>
                    </table>
            ...
            <h3 class="h3_title mt40">약사 신청 목록 ( <?=number_format($totalnum)?> )</h3>
            ...
            <div class="adm_table_style02">
                <table>
                    ...
                    if($totalnum > 0){
                        ...
                        $qry_002  = "SELECT t1.IDX as ID, t1.*, t2.*, t3.* " ;
                        ...
                        while($row_002 = $db->sql_fetch_array($res_002)){
                            ...
                            if($row_002['P_STATUS'] == "2") {
                                $pharmacist_status = "<font color='#3300FF'>$pharmacist_status_array[2]</font>";
                            } else if ($row_002['P_STATUS'] == "3") {
                                $pharmacist_status = "<font color='#FF6600'>$pharmacist_status_array[3]</font>";
                            }
                            ?>
                            <tr>
                                <td><?=$row_002['ss_date']?></td>
                                <td><?=$pharmacist_status?></td>
                                ...
                                <? if($row_002['P_STATUS'] == 2) { ?>
                                <td>
                                    <a onclick="cancle_pp('<?=$row_002["ID"]?>');" class="btn_s btn14">신청취소</a>
                                </td>
                                <? } else { ?>
                                    <td> &nbsp; </td>
                                <? } ?>
                        ...
                        ?>
                        <tr>
                            <td class="t_c" colspan="7" height="250">신청중인 약사가 없습니다.</td>
                        </tr>
                        <?php
                    }
                    ?>
            ...
            <div class="b_page_no">
                <?php
                $paging = new paging("./pharmacy.template.php","slot=pharmacist&type=pharmacist_apply_list",$offset,$page_block,$totalnum,$page,$url_opt);
                $paging->pagingArea("","") ;
                ?>
            </div>
...
<script>
    <!--
    function cancle_pp(idx){
        if(confirm("전문약사를 신청을 취소 하시겠습니까?")){
            var url = "./_action/pharmacist.do.php?Mode=cancle_pharmacist&idx="+idx ;
