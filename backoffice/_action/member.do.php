...
if ($Mode == "stipulation_update") {

    $idx = $_POST["idx"];
    $a_contents = add_escape($_POST["contents"]);
    $a_title = add_escape($_POST["title"]);

    $qry_001 = "UPDATE {$TB_STIPULATION} SET   ";
    $qry_001 .= "A_TITLE = '" . $a_title . "' ";
    $qry_001 .= ", A_CONTENTS = '" . $a_contents . "' ";
  ...
  $qry_001 = "SELECT COUNT(IDX) FROM {$TB_MEMBER} WHERE USER_ID = HEX(AES_ENCRYPT('".$user_email."','".SECRET_KEY."'))";
  ...
  if ($row_001[0] == 0) {
    ...
    <!--
      alert("아이디로 사용이 가능합니다");
      parent.document.getElementById("chk_id").value = "<?=$user_email?>";
      parent.document.getElementById("user_pass").focus();
    //-->
    </script>
    <?
    ...
    <!--
      ...
      parent.document.getElementById("chk_id").value = "";
      parent.document.getElementById("emailID").value = "";
      parent.document.getElementById("emailDomain").value = "";
      parent.document.getElementById("emailID").focus();
    //-->
    </script>
    <?
  ...
  exit;
    ...
    alert_js("alert_parent_move", "회원을 등록하였습니다", "../admin.template.php?slot=member&type=member_list");
    exit;
    ...
    $_link = "../admin.template.php?slot=member&type=member_list";
...
// 회원 수정
    ...
    echo "<br>================".$qry_001;
    echo "<br>================".$qry_002;
    alert_js("alert_parent_move", "회원을 수정하였습니다", "../admin.template.php?slot=member&type=member_list");
    exit;
...
// 회원 비밀번호 변경
    ...
    alert_js("alert_parent_move", "비밀번호를 수정하였습니다.", "../admin.template.php?slot=member&type=member_list");
    exit;
...
// 쪽지 삭제
    ...
    $qry_001 = "DELETE FROM {$TB_DM} WHERE IDX = '{$idx}' ";
