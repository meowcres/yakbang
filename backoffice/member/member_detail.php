...
if (isNull($_GET["idx"])) {
    alert_js("alert_back", "회원 정보가 옳바르지 않습니다.", "");
} else {
...
?>
<div id="Contents">
    <h1>회원 관리 &gt; 회원목록 &gt; <strong>회원상세정보</strong></h1>
    <div class="mt20"><b>○ 회원정보</b></div>
    ...
    <table>
        ...
        <tr>
            <th>상태</th>
            <td><?= $member_status_array[$m_main["USER_STATUS"]] ?></td>
        </tr>
        <tr>
            <th>생년월일</th>
    ...
    <div class="mt30 mb10">
        <input type="button" value="목록으로" class="Button btnGray w120"
               onClick="location.href='./admin.template.php?slot=member&type=member_list<?= $url_opt ?>'">&nbsp;
        <?
        foreach ($member_menu_array as $key => $val) {
            $_btn_class = $key == $_step ? "btnOrange" : "btnGray";
            ?><input type="button" value="<?= $val ?>" class="Button <?= $_btn_class ?> w120"
                     onClick="location.href='./admin.template.php?slot=member&type=member_detail&step=<?= $key . $url_opt ?>'">&nbsp;&nbsp;<?
        }
        ?>
    </div>
    ...
    <div>
        ...
        if (!isNull($_step)) {
            //echo $_step ;
            include_once "./member/detail.member_{$_step}.php";
        }
        ?>
    </div>
</div>
