...
$search["large"]   = isNull($_GET["search_large"])  ? "" : $_GET["search_large"];
$search["keyword"] = isNull($_GET["keyword"])       ? "" : $_GET["keyword"];

$where_array[] = "t1.CD_TYPE IN ('MENU')";
$where_array[] = "t1.CD_DEPTH IN ('2')";
...
$qry_from .= " LEFT JOIN {$TB_CODE} t2 ON (t1.PARENT_CODE=t2.CD_KEY AND t2.CD_DEPTH=1) " ;
